...
def main():
    ...
    epochs = 30
    best_acc = 0.0
    save_path = './train_weight/resNet34.pth'
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for val_data in val_bar:
                ...
                predict_y = torch.max(outputs, dim=1)[1]  # outputs:[batch_size,5], 16个样本，每个样本取最大值的标签
        ...
        print('[epoch %d] train_loss: %.3f  val_loss: %.3f val_accuracy: %.3f' %
              (epoch + 1, train_loss / train_steps, val_loss / val_steps, val_accurate))
        # tensorboard记录数据
        tb_writer.add_scalars('Loss', {'train': train_loss / train_steps, 'val': val_loss / val_steps}, epoch + 1)
        tb_writer.add_scalar('LR', optimizer.param_groups[0]["lr"], epoch + 1)
        tb_writer.add_scalar('Acc', val_accurate, epoch + 1)
